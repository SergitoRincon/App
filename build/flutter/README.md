...
* `motoapp` - project identifier (lowercase, no spaces, i.e. "snake_case") used for bundle IDs and package names.
* `` - project description.
* `MotoApp` - executable or bundle name on disk.
* `MotoApp` - display name shown in window titles and about app dialogs.
* `com.sergio` - org name in reverse domain name notation, e.g. `com.mycompany.myproject`.
